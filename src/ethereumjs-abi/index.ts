...
function encodeSingle(type, arg) {
  ...
  // console.log('encodeSingle', type, arg);
  if (type === 'address') {
...
function simpleEncode(method) {
  ...
  return Buffer.concat([
    methodID(sig.method, sig.args),
    rawEncode(sig.args, args),
  ]);
}

function simpleDecode(method, data) {
  const sig = parseSignature(method);
  ...
  return rawDecode(sig.retargs, data);
}

function _stringify(type, value) {
...
function stringify(types, values) {
  const ret = [];
...
function soliditySHA3(types, values) {
  return utils.keccak256(solidityPack(types, values));
}

function soliditySHA256(types, values) {
  return utils.sha256(solidityPack(types, values));
}

function solidityRIPEMD160(types, values) {
  return utils.ripemd160(solidityPack(types, values), true);
}
